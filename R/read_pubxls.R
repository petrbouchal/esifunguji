...
read_pubxls <- function(path) {
  ...
  esif <- read_excel(path, skip = 2, col_types = "text") %>%
    ...
    rename_with(~str_replace(.x, "financni_prostredky_vyuctovane_v_zadostech_o_platbu_", "fin_vyuct_v_zop_")) %>%
    rename_with(~str_replace(.x, "celkove_zpusobile_vydaje_pridelene_na_operaci_", "czv_")) %>%
    ...
    rename(prj_id = registracni_cislo_projektu_operace,
           ...
           real_zah_fyz = datum_zahajeni_fyzicke_operace,
