...
s7_categorised_prj %>%
  ...
  group_by(tema_id, tema_name) |> 
  summarise(czk_mil = round(sum(fin_vyuct_verejne)/1e6),
            n_proj = n_distinct(projname), .groups = "drop") %>% 
...
s7_categorised_prj %>%
  ...
  group_by(projname) %>%
  summarise(czk_mil = round(sum(fin_vyuct_verejne/1e6, na.rm = T)), 
            .groups = "drop") %>%
  arrange(desc(czk_mil)) %>%
  head(n = 10) %>% gt::gt()
```
...
s7_categorised_prj %>%
  ...
  group_by(tema_id) |> 
  slice_max(czk_mil, n = 10) |> 
  ungroup() |> 
  arrange(tema_id, desc(czk_mil)) %>%
  gt::gt()
```
